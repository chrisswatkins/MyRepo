rankhospital <- function (in_state, in_outcome, in_rank) {
        ...
        for (i in seq_along(ValidOutcome))  { 
                ...
                if (in_outcome == ValidOutcome[i]) {
                        ...
                        ## if valid outcome, Set column num of selected outcome
                        if (i == 1){DataIndex <- 11}
                        if (i == 2){DataIndex <- 17}
                        if (i == 3){DataIndex <- 23}
                        break
        ...
        ## Check for valid "ranking" value
        HospCnt <- nrow(outcome[outcome$State == in_state , ] )  ## Num of hosp in state

        if (is.numeric(in_rank) && in_rank > HospCnt) {return (NA)}     
        ...
        CompleteRows <- complete.cases(OrderedSet)
        AnswerSet <- OrderedSet[CompleteRows,]

        if (in_rank == "best") {in_rank <- 1}
        if (in_rank == "worst") {in_rank <- nrow(AnswerSet)}
        


        
        ans <- AnswerSet[in_rank,1]
        ans
